...
from snscrape._token import Token
...
class Tweet(snscrape.base.Item):
    ...
    tcooutlinksss = snscrape.base._DeprecatedProperty(
        "tcooutlinksss",
        lambda self: " ".join(self.tcooutlinks) if self.tcooutlinks else "",
        "tcooutlinks",
    )
...
class TwitterAPIScraper(snscrape.base.Scraper):
    def __init__(self, baseUrl, use_tor=None, **kwargs):
    ...
    def _get_token_with_tor(self):
        """Retrieve a twitter guest token.

        Used to request twitter from AWS"""
    ...
    def _ensure_guest_token(self, url=None):
        ...
        if self.use_tor:
            self.token_req = self._get_token_with_tor()
            r = self.token_req.res
        else:
            r = self._get(
    ...
    def _unset_guest_token(self):
        if self.token_req:
            self.token_req._close_tor_connexion()
            self.token_req = None
        self._guestToken = None
        ...
        for instruction in obj["timeline"]["instructions"]:
            ...
            for entry in entries:
                if entry["entryId"].startswith("sq-I-t-") or entry[
                    ...
                    yield from self._instruction_tweet_entry_to_tweet(
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def __init__(self, username, isUserId, **kwargs):
        ...
        self._baseUrl = (
            ...
            else f"https://twitter.com/i/user/{self._username}"
        )
    ...
    def _get_entity(self):
        ...
        params = {
            "variables": json.dumps(
                {fieldName: self._username, "withHighlightedLabel": True},
                separators=(",", ":"),
            )
        }
    ...
    def is_valid_username(s):
        ...
        ) or (s and s.strip(string.digits) == "")
...
class TwitterProfileScraper(TwitterUserScraper):
    ...
    def get_items(self):
        ...
        for obj in self._iter_api_data(
            f"https://api.twitter.com/2/timeline/profile/{userId}.json",
            params,
            paginationParams,
        ):
            yield from self._instructions_to_tweets(obj)
...
class TwitterTweetScraper(TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            obj = self._get_api_data(
                f"https://twitter.com/i/api/2/timeline/conversation/{self._tweetId}.json",
                params,
            )
            yield self._tweet_to_tweet(
                obj["globalObjects"]["tweets"][str(self._tweetId)], obj
            )
            ...
            for obj in self._iter_api_data(
                ...
                yield from self._instructions_to_tweets(
            ...
            while queue:
                ...
                for obj in self._iter_api_data(
                    f"https://twitter.com/i/api/2/timeline/conversation/{tweetId}.json",
                    params,
                    paginationParams,
                    direction=ScrollDirection.BOTH,
                ):
                    for tweet in self._instructions_to_tweets(
                        obj, includeConversationThreads=True
                    ):
    ...
    def from_args(cls, args):
        return cls(
            args.tweetId,
            TwitterTweetScraperMode.from_args(args),
            retries=args.retries,
            use_tor=args.use_tor,
        )
